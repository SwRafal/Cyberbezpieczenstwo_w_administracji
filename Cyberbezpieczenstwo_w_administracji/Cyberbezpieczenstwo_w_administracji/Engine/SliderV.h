...
namespace gm
{
	class SliderV : public sf::Drawable
	{
	private:
		int min_v;
		int max_v;
	...
	public:
		...
		SliderV() : SliderV(sf::Vector2f(0, 0), 200, 50) {};
		SliderV(const sf::Vector2f &position, float lenght, float width, int min_value = 0, int max_value = 100, int val = 0);
		~SliderV() {};
	};
}